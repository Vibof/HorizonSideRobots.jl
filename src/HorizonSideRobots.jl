...
Иначе просто создается объект, содержащий данные, определяющие эту обстановку (визуализации обстановки в этом случае нет).
...
Для подготовительной работы с sit-файлами (в них сохраняются данные с информацией о некоторой обстановке на поле с Роботом) 
...
function isborder(r::Robot,side::HorizonSideRobots.HorizonSide)::Bool
    ...
    if (i<1 && j<1)||(i<1 && j>jmax)||(i>imax && j>jmax)||(i>imax && j<1) # - (i,j) - за пределами фрейма (в угловых секторах)
        return false # ни в каких направлениях препятствий быть не может (т.к. на периметре фрейма не может быть перегородок, отличных от сплошной рамки)                
    elseif i<1 && 1<=j<=jmax # - (i,j) - за пределами фрейма (в северном секторе)
        if side ∈ (Nord,Sud)
            return false # в этих двух направлениях препятствий быть не может (т.к. на периметре фрейма не может быть перегородок, отличных от сплошной рамки) 
        end
        i,j = 1,j
    ...
    elseif r.situation.is_framed == true
        if side==Nord && i==1 || side==Sud && i==imax || side==West && j==1 || side==Ost && j==jmax
            return true # - Робот рядом с внешней рамкой и направление - в сторону рамки
        end
    end 
...
-- Открывает окно с текущей обстановкой на поле с Роботом, и предоставляет возможность редактирования
 ...
 Результат редактирования автоматически сохраняется в файле "untitled.sit" 
"""
function show_sit!(r::Robot)
    pre_show_sit_actions(r)
    SituationDatas.edit_sit!(r.situation,"untitled.sit")
    # обеспечена возможность редактирования с помощью мыши отображаемой обстановки и немедленного сохранения каждого акта редактирования в файле untitled.sit 
    return nothing 
end

function pre_show_sit_actions(r::Robot)
    if r.animate==true
        error("В режиме Robot(...;animate==true) невозможен вызов show_sit(::Robot,...) или show_sit!(::Robot,...)")
    end
