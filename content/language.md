...
Количество параметров у функций может быть любым, в том числе параметров может не быть совсем.
...
В рассмотренных примерах все параметры функций являются так называемыми **позиционными параметрами**. Это название происходит из того, что соответствие между формальными и фактическими параметрами устанавливается благодаря тому, что в списке параметров их позици фиксированы, т.е. каждому параметру определено строго определенное место в списке.

В языке Julia, однако, (как и в языке Python) наряду с позиционными параметрами у функций могут быть также и **именованные параметры** или их еще называют **ключевыми (key) параметрами**. Так называются параметры, за которыми прочно закреплены их имена. Например, известная нам функция Robot имеет именованный параметр `animate`. При вызове данной функции на исполнение этот параметр используется так
```julia
Robot(animate=true)
```
