...
РЕЗУЛЬТАТ: 
    -- Робот — в исходном положении (инвариант) в центре креста из маркеров, расставленных вплоть до внешней рамки.
"""
...
(предполагается, что это возможно — в противном случае произойдёт ошибка времени выполнения)
"""
...
где `Nord`, `Ost`, `Sud`, `West` — это символические имена, обозначающие стороны горизонта, а `0, 1, 2, 3` — соответствующие им целочисленные номера (в этом можно убедиться, воспользовавшись встроенной системой помощи Julia). 
